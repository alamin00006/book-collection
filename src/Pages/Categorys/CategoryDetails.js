...
const CategoryDetails = () => {
    ...
    const { isLoading, refetch} = useQuery([], () => fetch(`http://localhost:5000/api/v1/category/${categoryDetailsId}`, {
    method: "GET",
                 ...
                 <div className='p-5'>
                    <h2>{categoryDetails.name}</h2>
                     {categoryDetails?.products?.length > 0? <>
                        <h4>{categoryDetails.name} প্রকাশিত মোট {categoryDetails?.products?.length} টি বই পাচ্ছেন আমাদের কাছে</h4>
                    </>:<h4>{categoryDetails.name} প্রকাশিত কোন বই খুঁজে পাওয়া যায়নি</h4>}
                 </div>
             </div>
              <div className='my-card-main my-card'>
       {
            categoryDetails?.products?.map((data, index)=> <Cart key={data._id} data={data} ></Cart >) 
        }
