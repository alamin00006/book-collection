...
const ProductManage = () => {
  ...
  const { isLoading, refetch} = useQuery(['myProducts3',page,myProducts3,pageCount], () => fetch(`http://localhost:5000/api/v1/product?page=${page}&size=${3}`, {
      method: "GET",
  }).then(res =>res.json())
  .then(data =>{
    ...
    return (
      ...
      <tbody>
          ...
          {
             myProducts3?.map((product, index) => <ProductTable 
             product={product} index={index} setEditProduct={setEditProduct} handleShowEdit={handleShowEdit} handleShowDetails={handleShowDetails} setDeleteProduct={setDeleteProduct} productDelete={productDelete} refetch={refetch}
    ...
    <DetailsProductModal productDetails={productDetails} showDetails={showDetails} setShowDetails={setShowDetails} handleCloseDetails={handleCloseDetails}/>
      </div>
    )
};
